...
public class InfoPanel extends JPanel implements ActionListener{
	...
	public void showInfo(DTNHost host) {
		...
		reset();
		/**金程皓**/
		DecimalFormat df = new DecimalFormat( "0.0000 ");  //限制小数后面保留的位数
		this.selectedHost = host;
		double x = host.getLocation().getX();//x
		double y = host.getLocation().getY();//y
		double z = 50+Math.random()*150;//z
		...
		routingInfoButton = new JButton("路由信息");
