...
public class SimScenario implements Serializable {
	...
	/*********金程皓*******************/
	//自定义的属性
	public static final String GROUP_NODE_MA = "groupNodeMaster";
	public static final String GROUP_NODE_COR = "groupNodeCorn" ;
	public static final String GROUP_NODE_DIS = "groupNodeDis";
	public static final String GROUP_Speed_Level = "groupSpeedLevel";
	...
	protected void createHosts() {
		...
		for (int i=1; i<=nrofGroups; i++) {
			...
			/******金程皓************/
			//获取这个Group中每个节点的标志信息
			String groupNodeMaster = s.getSetting(GROUP_NODE_MA);
			String groupNodeCorn = s.getSetting(GROUP_NODE_COR);
			String groupNodeDis = s.getSetting(GROUP_NODE_DIS);
			int groupSppedLevel = s.getInt(GROUP_Speed_Level);
			...
			/*************金程皓***************************/
			//UAVNet设置
			UAVNet uavNet = new UAVNet(gid,groupSppedLevel);
			uavNet.setGroup_master(groupNodeMaster);
			uavNet.setGroup_NodeDistance(groupNodeDis);
			uavNet.setGroupNodeCorn(groupNodeCorn);
			...
			for (int j=0; j<nrofHosts; j++) {
				...
				DTNHost host = new DTNHost(this.messageListeners, 
						...
						mmProto, mRouterProto,uavNet.getGroup_master()[j],
						uavNet.getGroupNodeCorn()[j],uavNet.getGroup_NodeDistance()[j],
						uavNet.getGroup_address()*256 + j,uavNet.getGroup_Speed_Level());
				hosts.add(host);
			}
			uavNet.setHosts(hosts);
		}
	}
